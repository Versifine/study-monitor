...
- 当前状态：M0、M1、M2、M3、M4、M5 已完成；M6 认证工具已实现，真实 14 天尚未开始
...
当前已实现 M1 的 SQLite 仅追加事件存储与本地 API，M2 的外部短视频受管导入，M3 的 ActivityWatch GET-only 适配、通用 JSON Evidence、追加心跳、按计划覆盖率和统一时间线，M4 的 Windows 有界自动恢复、磁盘保护、默认关闭的严格媒体保留、可验证备份/新目录恢复和兼容应用回滚，以及 M5 构建后嵌入 Go 二进制、无需开发服务器的最小只读仪表盘。仍不包含自动升级、云端必需备份、分析队列或 AI。

## 你现在应该做什么

先审查 M5 的前端状态映射、API 契约、离线构建、浏览器验收、smoke 和 Git 检查点，然后停止。只有在单独确认后，才把 [`codex-prompts/07_M6.txt`](codex-prompts/07_M6.txt) 作为下一次任务；不要提前开始 14 天认证或进入 AI。

## M5 本地命令

要求 Windows PowerShell 5.1、Go 1.25.4；只有重建/校验前端源码时需要固定 Node.js 24.11.1，生产运行不需要 Node.js：
...
默认仅监听 `127.0.0.1:47831`。`GET /health/live` 只表示进程存活；`GET /health/ready` 检查 SQLite 与数据库保留空间是否允许确认写入；`POST /api/v1/events/batch` 和 `/api/v1/evidence/batch` 接收同合同版本化幂等事件；`POST /api/v1/collectors/heartbeats/batch` 追加心跳；`GET /api/v1/timeline` 与 `/api/v1/coverage` 提供稳定时间线和明确缺口；`GET /api/v1/operations/status` 暴露磁盘与保留状态；`GET /` 和 `GET /api/v1/dashboard/summary` 提供只读仪表盘与有界汇总。完整合同见 [`docs/API.md`](docs/API.md)、[`docs/DASHBOARD.md`](docs/DASHBOARD.md)、[`docs/COLLECTORS_AND_TIMELINE.md`](docs/COLLECTORS_AND_TIMELINE.md) 与 [`docs/OPERATIONS.md`](docs/OPERATIONS.md)。
...
媒体入口默认关闭。显式启用并配置固定版本 ffprobe 后，外部采集器按媒体、sidecar、ready 三文件合同发布分段；`GET /api/v1/media/ingest/status` 显示模块状态、入口积压和字节需求。完整合同见 [`docs/MEDIA_INGEST.md`](docs/MEDIA_INGEST.md)。
...
- `EXAM_MONITOR_MODE`
- `EXAM_MONITOR_DASHBOARD_ENABLED`
...
- `EXAM_MONITOR_MEDIA_INGEST_ENABLED`
- `EXAM_MONITOR_MEDIA_INBOX_DIRECTORY`
- `EXAM_MONITOR_MEDIA_SCAN_INTERVAL`
- `EXAM_MONITOR_MEDIA_SETTLE_INTERVAL`
- `EXAM_MONITOR_MEDIA_MAX_SEGMENT_BYTES`
- `EXAM_MONITOR_MEDIA_MAX_SEGMENT_DURATION`
- `EXAM_MONITOR_MEDIA_MAX_SIDECAR_BYTES`
- `EXAM_MONITOR_MEDIA_MAX_SCAN_ENTRIES`
- `EXAM_MONITOR_FFPROBE_PATH`
- `EXAM_MONITOR_FFPROBE_TIMEOUT`
