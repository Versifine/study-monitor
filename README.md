...
当前已实现 M1 的 SQLite 仅追加事件存储与本地 API，以及 M2 的外部短视频 sidecar/ready 导入、受管复制、完整性验证、隔离、幂等确认和恢复。仍不包含 ActivityWatch 适配、覆盖率、前端、运维自动化或 AI。

## 你现在应该做什么

先审查 M2 的测试、构建、smoke 和 Git 检查点，然后停止。只有在单独确认后，才把 [`codex-prompts/04_M3.txt`](codex-prompts/04_M3.txt) 作为下一次任务；不要提前进入 ActivityWatch 适配、覆盖率、前端、运维自动化或 AI。
