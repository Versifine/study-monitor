...
- 权威事实：`raw_events`、`collector_heartbeats`、媒体导入/身份/状态事件，以及 M4 故障、模块状态和保留操作事件。业务内容只追加，不原地改写或覆盖。
...
M3 的 ActivityWatch 适配器只通过 loopback HTTP GET 读取一个配置绑定的 bucket，不直接打开或修改其数据库。每个 bucket 独立轮询、限时、限页和保存 Core 侧断点；失败只改变本采集器状态。通用 JSON 复用 M1 写入合同，心跳使用独立追加 schema。完整边界见 [`COLLECTORS_AND_TIMELINE.md`](COLLECTORS_AND_TIMELINE.md)。

### M3 时间线与覆盖率

统一时间线是 SQLite 内的可重建投影，将原始事件、心跳和已接受媒体规范成共同时间字段。投影保存固定精度校正时间用于稳定快照分页，但始终返回原始设备字符串、设备 UTC、接收 UTC、偏移和误差。

覆盖率计算使用 `internal/coverage` 的纯确定性逻辑，按本地时区计划窗口展开后，以明确心跳、ActivityWatch 时段和媒体段覆盖缺口。每个采集器独立事务替换投影；无有效 idle 事实时禁止 `confirmed_idle`。时间线/覆盖率失败均不回滚事实写入。

### M4 运维与恢复

`internal/operations` 在同一 Go 进程中负责可注入磁盘探测、媒体/核心写入门、WAL checkpoint、严格白名单临时文件收敛、默认关闭的媒体保留和只读状态。它不接管外部来源文件，不创建通用清理器；普通故障只降低本模块状态。高风险备份、恢复、版本切换和 Task Scheduler 生命周期保留在本机 PowerShell 脚本中，避免增加常驻服务和管理 API。

Windows 生产任务运行 `run-supervised.ps1`，由 `current.json` 解析固定发布目录和配置。Task Scheduler 重试与跨启动崩溃窗口都是有界的。每个发布目录包含二进制、配置和 `release-manifest.json`；`previous.json` 只表示上一应用版本，不携带或恢复旧数据库快照。

### M5 嵌入式只读仪表盘

`web/src` 是 TypeScript/HTML/CSS 源，固定 Node.js 的无依赖构建把确定性产物写到 `web/dist`，随后由 `web/assets.go` 嵌入同一个 Go 二进制。生产没有 Node.js、开发服务器、外部字体或网络资源依赖。`internal/dashboard` 只服务固定资源白名单；`internal/httpapi` 只在处理器可用时注册根页面和有界汇总。

仪表盘按顺序查询汇总、时间线和覆盖率，不自动轮询、不并发占用单投影门，也不建立新队列或持久化状态。环境变量可独立关闭处理器，Minimum mode 强制关闭。资源加载或页面查询失败不会参与 readiness，也不会停止事实写入、采集、恢复和 M4 运维。完整合同见 [`DASHBOARD.md`](DASHBOARD.md)。
...
│  ├─ app/                      # 启停、模块编排、降级
│  ├─ config/                   # 配置加载、默认值和校验
│  ├─ eventstore/               # 仅追加事实、独立迁移账本与 SQLite 投影/查询
│  ├─ mediaingest/              # sidecar、暂存、校验和隔离
│  ├─ collectors/               # ActivityWatch GET-only 适配；通用 JSON 由 httpapi 复用 M1 合同
│  ├─ coverage/                 # 可重建覆盖率投影
│  ├─ operations/               # M4 磁盘门、保留、WAL/临时收敛和故障事实
│  ├─ dashboard/                # M5 嵌入静态资源的 GET/HEAD-only 处理器
│  ├─ httpapi/                  # 本地 API 和只读查询
│  └─ logging/                  # JSON 日志与有界文件轮转
├─ migrations/                 # 只前向、兼容上一稳定版回滚
├─ web/
│  ├─ assets.go                 # 将已校验 dist 嵌入生产二进制
│  ├─ build.mjs                 # 固定 Node.js、零依赖确定性构建
