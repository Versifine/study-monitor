...
- 核心事件 schema 使用 `PRAGMA user_version` 与 `schema_migrations`；M2 媒体新增结构使用独立的 `media_schema_migrations`，不提高核心版本号。两个账本都只追加并校验嵌入 SQL 的 SHA-256
- M2 数据库保持核心版本 `1`；上一稳定 M1 应用只读取核心账本和已知表，可以安全忽略媒体表与媒体迁移账本
- 所有版本和校验和写入构建/冻结清单
