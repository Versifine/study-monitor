...
- 核心事件 schema 使用 `PRAGMA user_version` 与 `schema_migrations`；M2 媒体新增结构使用独立的 `media_schema_migrations`；M3 心跳、断点和投影使用独立的 `m3_schema_migrations`。三个账本都只追加并校验嵌入 SQL 的 SHA-256，M2/M3 都不提高核心版本号
- M3 数据库保持核心版本 `1`；上一稳定 M2 应用只读取核心/媒体账本和已知表，可以安全忽略 M3 表与迁移账本
- 所有版本和校验和写入构建/冻结清单
