...
- schema v1 列出的全部字段都必须显式存在；`schema_version` 必须是 `1`，`complete` 必须是 `true`，缺失 `complete` 不等同于未完成状态。
...
7. 数据库提交成功后，向来源入口原子写入确认标记。后续扫描只有在确认标记与当前 sidecar、来源媒体、数据库声明和受管媒体同时一致时才跳过该 ready；复制来的确认、陈旧确认和受管文件缺失都必须重新进入收敛流程。

数据库绝不先于受管文件确认 `accepted`。若进程在目标改名后、数据库提交前退出，来源仍无确认；重启后同一 ready 分段会校验既有受管文件并收敛到同一媒体记录。若已登记的固定 `accepted/<sha256>.media` 意外缺失，仍完整的来源会经 staging 重新校验并恢复该文件；既有但内容错误的目标不会被静默覆盖。若权威事实已提交而投影更新失败，事实不回滚；重试或启动时的投影重建完成收敛。
...
导入状态保持只追加：相邻的完全相同状态重试复用同一事实；若状态经过其他代际后再次回到 `accepted` 或 `quarantined`，必须追加新的状态事实，使当前投影和从历史重建的投影都能反映恢复后的最新状态。
...
```json
{
  ...
  "collector_id": "desk.camera",
  "source_idempotency_key": "desk-20260719-100000-0001",
  "sidecar_fingerprint": "2222222222222222222222222222222222222222222222222222222222222222",
  "media_segment_id": 1,
  "sha256": "346f4da339e0f6f91e1785436c74f97a42b87392e6f4939b2fc01b5e12f442d8",
...
`sidecar_fingerprint` 由 Core 对完整 canonical sidecar 计算并写入，Core 重扫时还会与数据库中已接受媒体对应的 sidecar 字段重新计算结果核对，不能由来源声明替代。来源应至少核对 schema、collector、来源幂等键、媒体 ID 和 SHA-256。确认缺失或损坏时，可以保留原三文件并安全重放。

## 6. 隔离和错误

损坏、伪造或不合规媒体不会进入 `accepted`。Recorder Core 在受管 `quarantine/` 中保存媒体副本、可用的原 sidecar 和 `*.reason.json`；来源文件保持不变。既有确定性隔离目标必须是与当前候选完全一致的普通文件，否则记录 `MEDIA_QUARANTINE_FAILED`。同一运行期内，来源、sidecar 和已验真的隔离副本均未变化时不会重复复制或探测；任一发生变化或隔离副本缺失时重新验证、修复或显式失败。隔离本身失败时不伪造隔离或接受成功。
...
ffprobe 缺失或版本不匹配只把媒体模块设为 unavailable。每轮扫描先重新确认固定版本；初始化后的可执行文件缺失、替换、超时或数据库故障同样在媒体状态中显示 unavailable，并保留来源和 staging 等待重试，不把合法媒体误报为已隔离。后台按配置的有界扫描间隔继续重试，下一次完整扫描成功后恢复 healthy。M1 事件写入、查询和 `/health/ready` 继续工作；媒体失败不能把核心事件存储伪装为不可用。
...
配置限制扫描间隔、稳定等待、单段字节数、单段时长、sidecar 字节数、每轮最多处理的 ready 条目和 ffprobe 超时。默认最多处理 2 GiB、10 分钟的分段，每轮最多 1000 个 ready 条目；扫描在内存中只保留与该上限成比例的候选，已验真的确认/隔离文件戳签名保存在可重建的 SQLite 投影中，不会因进程重启或内存裁剪而忘记。游标在有序 ready 集合中轮转，长期 pending 条目不能饿死后续条目，首次确认验真同样计入每轮上限。部署时可以收紧上限，不能把时长放宽到超过 10 分钟。
